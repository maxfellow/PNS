// ============================================================
// Padmamma Narayana Shetty Global Foundation v2 — motion layer
// ============================================================
...
/* (tilt is now a simple CSS lift on hover — no JS tracking needed, keeps it calm) */

/* ---------- staggered grid entrance (adds motion without harsh hover) ---------- */
document.querySelectorAll('.teaser-grid, .gallery-grid, .involve-grid-light, .trustee-grid, .story-grid, .involve-grid').forEach(grid=>{
...
if (donateForm) donateForm.addEventListener('submit', (e)=>{
  ...
  const msg = `Hi Padmamma Narayana Shetty Global Foundation, I would like to make a donation.\n\nName: ${f.get('name')}\nAmount: Rs. ${f.get('amount')}\nContact: ${f.get('contact')}\nMessage: ${f.get('message') || '-'}`;
  openWhatsApp(msg);
...
if (volunteerForm) volunteerForm.addEventListener('submit', (e)=>{
  ...
  const msg = `Hi, I would like to volunteer with Padmamma Narayana Shetty Global Foundation.\n\nName: ${f.get('name')}\nPhone: ${f.get('phone')}\nArea of interest: ${f.get('interest')}\nAvailability: ${f.get('availability') || '-'}`;
  openWhatsApp(msg);
...
if (partnerForm) partnerForm.addEventListener('submit', (e)=>{
  ...
  const msg = `Hi, our organisation is interested in partnering with Padmamma Narayana Shetty Global Foundation.\n\nOrganisation: ${f.get('org')}\nContact person: ${f.get('name')}\nEmail: ${f.get('email')}\nDetails: ${f.get('details') || '-'}`;
  openWhatsApp(msg);
